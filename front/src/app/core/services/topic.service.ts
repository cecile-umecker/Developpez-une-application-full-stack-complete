...
export class TopicService {
  ...
  getAllTopics() {
    return this.http.get<Topics>('/api/topic').pipe(
  ...
  getUserTopics() {
    return this.http.get<Topics>('/api/topic/my').pipe(
      tap(data => this.topicsSubject.next(data))
    );
  }
  ...
  subscribeToTopic(topicId: number) {
    return this.http.post<Topic>(`/api/topic/${topicId}/subscription`, {}).pipe(
  ...
  unsubscribeFromTopic(topicId: number) {
    return this.http.delete<Topic>(`/api/topic/${topicId}/subscription`).pipe(
      tap(updatedTopic => this.updateTopicInList(updatedTopic))
    );
  }
